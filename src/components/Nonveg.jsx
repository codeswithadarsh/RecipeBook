...
function Nonveg() {
  ...
  const getNonvegRecipes = async () => {
    const check = localStorage.getItem("nonveg");

    if (check) {
      setNonvegRecipes(JSON.parse(check));
    } else {
      const api = await fetch(`https://api.spoonacular.com/recipes/random?apiKey=${process.env.REACT_APP_API_KEY}&number=10&tags=non-vegetarian`);
